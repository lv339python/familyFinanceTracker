...
from ibudget.settings import CLIENT_SECRET, CLIENT_ID, AUTHORIZATION_BASE_URL, LOCAL_URL, SCOPE, REDIRECT_URL, TOKEN_URL
...
def google_auth_grant(request):
    ...
    :param request: request from the website
    :return: redirect to GoogleOauth2 API if Oauth2 session object generated successful, status 400 if unsuccessful
    """
...
def google_sign_in(request):
    ...
    google = OAuth2Session(CLIENT_ID, scope=SCOPE, redirect_uri=REDIRECT_URL)
    try:
        google.fetch_token(TOKEN_URL, client_secret=CLIENT_SECRET, authorization_response=LOCAL_URL + request.
                           get_full_path(), code=request.GET["code"])
    except:
        return HttpResponse(status=400)

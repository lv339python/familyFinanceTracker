...
def find_users(members_id):
    ...
    for member_id in members_id:
        member = UserProfile.get_by_id(member_id)
        if member:
...
def create_group_dict(data, user):
    """
    ...
    icon = data.get('icon')
    if string_validator(icon, 0, 300):
        result['icon'] = icon
    return result
...
def get_users(data, user):
    ...
    owner = UserProfile.get_by_id(owner_id) if isinstance(owner_id, int) else None
...
def update_group_dict(data, user):
    """
